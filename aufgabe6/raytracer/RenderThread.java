...
/**
 *
 * @author Mark Deuerling, Peter Albtecht
 */
...
//        ThreadFactory();
        ...
        //initialize JProgressBar
        bar = new ProgressBar(frame.getWidth());
        bar.renderProgress();  
    }
    ...
    final protected void renderPixel(int dx, int dy, int width, int height){
        ...
        for(int x = dx; x < width; x++){
            ...
            try{
                oldTime = bar.updateBar(oldTime);
            }catch(NullPointerException e){
               System.out.println("Swing is not thread-safe!");
            }
            repaint();
        }
    }
